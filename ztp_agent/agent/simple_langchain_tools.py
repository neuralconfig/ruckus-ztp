"""
Simplified LangChain tools for the ZTP AI agent using factory functions.
"""
import logging
...
def set_network_context(switches: Dict[str, SwitchOperation], ztp_process=None):
    """Set the network context for all tools."""
    global _switches, _ztp_process
    _switches = switches
...
def run_show_command(switch_ip: str, command: str) -> Dict[str, Any]:
    """Execute diagnostic 'show' commands on a RUCKUS ICX switch.
    ...
    try:
        ...
        logger.error(f"Connection error running command '{command}' on switch {switch_ip}: {e}")
        return {
            "success": False,
    ...
    for ip in _switches.keys():
        ...
        try:
            with _switches[ip]:
                ...
                success, output = _switches[ip].run_command("show version")
                if success:
...
def get_switch_details(switch_ip: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.error(f"Connection error getting switch details for {switch_ip}: {e}")
...
def get_network_tools(switches: Dict[str, SwitchOperation], ztp_process=None) -> List:
    """Get all network tools after setting the context."""
    # Set the global context
    set_network_context(switches, ztp_process)
